...
| **次按钮 (Secondary)**| **默认 (Default)** |`--color-blue-2`| `--color-blue-5` | 无| 表格内操作、表单的“取消”、次级引导。用于视觉降噪。 |
| *(中视觉权重)* | **悬停 (Hover)** | `--color-blue-3`|  `--color-blue-5` | 无 | 次按钮的鼠标悬停反馈，背景色阶加深一层。 |
...
4. **尺寸与内边距自适应约束 (Size & Padding Auto-layout):**
   - **固定高度:** 默认情况下，主按钮和次按钮的高度固定为 `48px`。
   - **自适应宽度:** 按钮的 `width` 严禁写死固定值，必须随内部文字长度自适应拉伸。
   - **对齐与留白规范:** 为了保证文字绝对居中且左右留白一致，AI 必须使用以下标准的 CSS 范式：
     ```css
     .btn {
       height: 48px;
       padding: 0 24px; /* 保持左右 24px 的安全间距 */
       display: inline-flex;
       align-items: center;
...
### 3.2 间距 Token 与卡片阵列规范 (Spacing Tokens & Grid Gaps)

| Token | 值 | 用途 |
|---|---|---|
| `--spacing-xs` | `4px` | 极小间距、图标与文字间距 |
| `--spacing-sm` | `8px` | 小间距、紧凑布局（如列表项内） |
| `--spacing-md` | `16px` | 默认边距、中等间距、**元素内边距及卡片网格槽宽 (Gutter)** |
| `--spacing-lg` | `24px` | 大间距、卡片内边距 |
| `--spacing-xl` | `32px` | 超大间距、区块内细分间距 |
| `--spacing-2xl`| `48px` | 极大间距、大区块间距 |
| `--spacing-3xl`| `64px` | 页面级间距（如移动端模块间距） |

#### 💡[AI Agent 识别与生成规则]：卡片阵列的间距推算与布局 (Grid Layout Spacing)
- **禁止硬编码宽度：** 当遇到如“2行3列”等同层级卡片并排展示时，**严禁**手动为卡片计算和写死绝对宽度（如 `width: 380px`）。
- **统一卡片间距：** 同层级卡片之间（无论是左右相邻还是上下相邻），**默认必须保持 16px 的间距**。
- **强制 Grid 响应式范式：** 必须使用 CSS Grid 布局，将 16px 间距交给浏览器自动计算。AI 遇到卡片组时必须输出如下结构：
  ```css
  .card-grid-container {
    display: grid;
    grid-template-columns: repeat(3, 1fr); /* 假设为3栏，按设计稿自适应 */
    gap: var(--spacing-md); /* 核心：强制使用 16px 作为上下左右间距 */
  }
...
#### [AI Agent 识别与生成规则]：模块级垂直留白:**
   - - **网页端默认 80px 铁律：** 在没有特别声明的情况下，页面级别的两个大业务模块（如 HTML 中的 `<section>` 标签层级）之间，**必须**保持 `80px` 的留白，禁止模块紧密贴合。
- **⚠️ 防间距翻倍约束 (No Double Spacing Rule)：** 为了防止相邻容器各自带有 80px 导致最终间距叠加变成 `160px`，AI 必须严格采用以下两种现代 CSS 方案之一来处理大模块间距：
  
  **方案 A (推荐)：父容器统筹法 (Flex/Grid Gap)**
  将所有的 `<section>` 包裹在一个大容器中，由父容器统一分配间距，彻底杜绝叠加风险：
  ```css
  .main-content-wrapper {
    display: flex;
    flex-direction: column;
    gap: var(--section-gap-desktop); /* 强制统一使用 80px */
  }
---
**方案 B (推荐)：单向向下推排法 (Single-direction Margin)**
如果不用 Flex 容器，则只能统一下发底边距，绝对禁止相邻模块同时写 margin-bottom 和 margin-top 或 padding：
 ```css
.section-module {
  margin-bottom: var(--section-gap-desktop); /* 统一向下推 80px */
}
.section-module:last-child {
  margin-bottom: 0; /* 最后一个模块清除底边距 */
}
 ```
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| 1.0 | 2026-04-29 | 初始版本，基于 SurferCloud 设计规范图片创建 |
